   ...
   fMateIDs(nullptr)
{
  ...
  fSection = "full"; // Head partial phantom is the default option
}
...
//----- Store materials in a vector
  ...
  // Rivelatore
    G4double detector_sizeXY = 70. * cm;
    G4double detector_sizeZ = 1.0 * cm;
    G4Box* solidDetector = new G4Box("Detector", detector_sizeXY / 2, detector_sizeXY / 2, detector_sizeZ / 2);
    G4LogicalVolume* logicDetector = new G4LogicalVolume(solidDetector, silicon, "Detector");
    new G4PVPlacement(0, G4ThreeVector(0, 0, 80.5 * cm), logicDetector, "Detector", logicWorld, false, 0);  
...
//----- Define the volume that contains all the voxels
  ...
  G4cout << " placing voxel container volume at -72.5" << posCentreVoxels << G4endl;

    G4RotationMatrix* rotCylinder = new G4RotationMatrix();
    rotCylinder->rotateX(90 * deg); // Ruota di 90 gradi attorno all'asse X
rotCylinder->rotateZ(330 * deg); // Modified rotation

   
  fPhantomContainer
  = new G4PVPlacement(rotCylinder,                     // rotation
                      G4ThreeVector(0,-72.5*cm,0),
